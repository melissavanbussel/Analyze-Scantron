#' Non-Functional Distractors (Positive Correlations)
#'
...
#'non-functional distractors which had positive correlations with student scores for that question.
#'
#'@export

non_functional_distractors_pos_cor <- function(exam, numchoicesperitem = NULL, check_format = TRUE) {
if (check_format == TRUE) {
  stopifnot(correct_format(exam) == TRUE)
}
num_items <- num_items(exam, check_format = FALSE)
distractor_correlations <- distractor_correlations(exam, numchoicesperitem, check_format = FALSE)
non_functional_distractors_pos_cor <- list(length = num_items)
for (i in 1:num_items) {
  non_functional_distractors_pos_cor[[i]] <- distractor_correlations[[i]][distractor_correlations[[i]] >= 0 | is.na(distractor_correlations[[i]])]
}
return(non_functional_distractors_pos_cor)
}
