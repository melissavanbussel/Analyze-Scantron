...
#'however, that increasing the number of questions on a test will always increase the reliability of the test. Thus,
#'the Adjusted Cronbach Alpha is a measure of what the reliability of the test would be if the test had had 50
#'questions. This makes it easy to compare reliabilities across tests of different lengths.
#'
...
#'
#'@seealso \code\link{cronbach_alpha}
#'
#'@export


adjusted_cronbach_alpha <- function(exam, check_format = TRUE) {
  ...
  cronbach_alpha <- cronbach_alpha(exam, check_format = FALSE)
  return( ((50 / num_items)*(cronbach_alpha))/(1+((50.0/num_items)-1)*cronbach_alpha) )
}
