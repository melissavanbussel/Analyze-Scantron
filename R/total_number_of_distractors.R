...
#'
#' Finds the total number of distractors on the test.
#'
...
#'
#'@details A distractor is defined as any option on a multiple choice question which is not the correct answer. For
#'example, if a question had options A, B, C, and D, and the correct answer was B, then the distractors for that
#'question would be A, C and D.
#'
...
#'the distractors for the given question, along with the proportion of students who chose each of the distractors
...
total_number_of_distractors <- function(exam, numchoicesperitem = NULL, check_format = TRUE){
  ...
  return(sum(num_choices_per_item(exam, numchoicesperitem, check_format = FALSE) - 1))
}
